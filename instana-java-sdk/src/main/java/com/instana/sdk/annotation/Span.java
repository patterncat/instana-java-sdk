...
/**
 * <p>
 * Declaring this annotation on any method triggers a recording of this method's runtime and some basic properties. Such
 * traces cannot be nested where only the inner-most method's runtime is recorded.
 * </p>
 * <p>
 * If the recording should end upon reaching a different method, the {@link Start} and {@link End} annotations should be
 * used instead. It is <b>not</b> required to include this annotation in the final application and running the
 * application without an active Instana agent will not affect the functionality of the annotated method.
 * </p>
 */
...
public @interface Span {
  ...
  /**
   * @return The name of the captured span which is used for grouping spans and is used as a marker in the GUI.
   */
  String value();

  /**
   * @return Indicates the type of the span that this method implements. A span can either capture an exit where this
   *         process currently communicates with an external resource such as a database. An entry typically describes a
   *         scenario where this VM process a command from an external application.
   */
   ...
   * The tracing is continued until a method declaring an {@link End} annotation is reached. Such traces cannot be
   * nested where only the inner-most method's runtime is recorded.
   * </p>
   * <p>
   * If the recording should when completing this method, the {@link Span} annotation should be used instead. It is
   * <b>not</b> required to include this annotation in the final application and running the application without an
   * active Instana agent will not affect the functionality of the annotated method.
   * </p>
   */
  @Retention(RetentionPolicy.RUNTIME)
  @Target(ElementType.METHOD)
   ...
   * needs to declare both the same name and {@link Type} as the related exit.
   */
  @Retention(RetentionPolicy.RUNTIME)
  @Target(ElementType.METHOD)
  @interface End {
